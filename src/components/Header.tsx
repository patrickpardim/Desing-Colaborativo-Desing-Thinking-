  ...
  return (
    <header id="header_container" className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 shadow-xs z-10">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="hidden sm:block">
          ...
          <p id="header_facilitator" className="text-[10px] text-slate-400 mt-0.5 font-medium">
            Facilitador: <span className="text-slate-600">@{room.facilitatorName}</span>
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center bg-slate-100 rounded-full px-4 py-1.5 gap-4 md:gap-6 border border-slate-200/50">
          ...
          {currentUser.isFacilitator ? (
            <div className="flex items-center gap-1.5">
              <button
                id="btn_toggle_timer"
                onClick={toggleTimer}
                className="p-1 hover:bg-slate-200 rounded transition-all text-slate-600 hover:text-slate-900"
                title={room.timerActive ? 'Pausar Cronômetro' : 'Iniciar Cronômetro'}
              >
            ...
            <div className="flex items-center gap-1">
              ...
              {room.timerSeconds === 0 && (
                <span className="text-xs text-rose-500 flex items-center gap-0.5 animate-bounce font-bold">
                  <AlertCircle className="w-3.5 h-3.5" /> Tempo Esgotado!
                </span>
              )}
            </div>
          )}
        ...
        <div className="flex items-center gap-1 text-xs md:text-sm font-semibold tracking-wider text-slate-700">
          ...
          <button
            ...
            className="p-1 hover:bg-slate-200 rounded transition-all ml-0.5"
            title="Copiar Código PIN da Sala"
          >
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <Download className="w-3.5 h-3.5" /> Exportar Sessão
        </button>
        ...
        {currentUser.isFacilitator ? (
          <>
            ...
            <button
              ...
              title="Sair temporariamente da sala"
            >
            ...
            >
              <X className="w-3.5 h-3.5" /> Encerrar Sala
            </button>
          </>
        ) : (
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl border border-slate-100 animate-in fade-in zoom-in-95 duration-200 text-left">
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1">
                <h3 className="text-base font-extrabold text-slate-800">Sair sem encerrar?</h3>
                <p className="text-xs text-slate-500 leading-relaxed">
                  Você sairá desta sala, mas as interações e os post-its continuarão ativos para os alunos e demais usuários.
                </p>
                <div className="bg-amber-50/50 border border-amber-100 rounded-xl p-3 mt-2">
                  <p className="text-[11px] text-amber-800 font-medium">
                    💡 <strong>Como voltar depois:</strong> Você poderá retornar futuramente digitando o PIN <span className="font-mono font-bold text-amber-900">{room.pin}</span> e utilizando o seu nome de facilitador exato: <span className="font-bold text-amber-950">@{room.facilitatorName}</span>.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl border border-slate-100 animate-in fade-in zoom-in-95 duration-200 text-left">
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1 animate-in">
                <h3 className="text-base font-extrabold text-slate-800">Realmente encerrar a sala?</h3>
                <p className="text-xs text-rose-600 font-extrabold flex items-center gap-1">
                  ⚠️ ESTA AÇÃO É IRREVERSÍVEL!
                </p>
                <p className="text-xs text-slate-500 leading-relaxed">
                  Isso excluirá permanentemente todos os dados desta sala, incluindo post-its de ideias, participantes cadastrados e configurações. Todos os alunos ativos serão desconectados imediatamente.
                </p>
              </div>
            </div>
